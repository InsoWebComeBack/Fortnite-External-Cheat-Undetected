...
bool intel_driver::ClearMmUnloadedDrivers(HANDLE device_handle)
{
	...
	while (status == nt::STATUS_INFO_LENGTH_MISMATCHING_LOL)
	{
...
std::string intel_driver::random_driver_string(const int len)
{
	const std::string alpha_numericc("ABCDEFGHIJKLMNOPRSTUVZ01234567899874563210");
	std::default_random_engine generatorr{ std::random_device{}() };
	const std::uniform_int_distribution< std::string::size_type > distributionn{ 0, alpha_numericc.size() - 1 };
	std::string strr(len, 0);
	for (auto& itt : strr) {
		itt = alpha_numericc[distributionn(generatorr)];
	}
	return strr;
}
